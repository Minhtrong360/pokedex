...
router.post("/pokemons", function (req, res, next) {
  ...
  const { name, id, types, url, height, weight, description, abilities } =
    req.body;
  ...
  const newPokemon = {
    ...
    abilities: abilities ? abilities : faker.company.catchPhraseAdjective(),
  };
  ...
  pokemons?.data?.map((pokemon) => {
    ...
    // else {
    //   // pokemons.data = pokemons.data.push(newPokemon);
    //   // pokemons.totalPokemons = pokemons.data.length;
    //   // console.log("pokemons mới", pokemons);
    //   // fs.writeFileSync(
